...
export default function InvitationsPage() {
  ...
  const pending = invitations.filter((i) => i.status === 0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          ...
          {pending.length > 0 && <span className="ml-2 bg-blue-600 text-white text-xs px-1.5 py-0.5 rounded-full">{pending.length}</span>}
        </p>
      </div>
